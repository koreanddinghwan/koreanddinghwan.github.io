...
- 장점
	- 극단적인 예시로는, `분산 시스템에서 전체 DB를 모든 site에 복사`해 `fully replicated distributed database`를 만드는 것.
...
1. Query Mapping
	- 분산 데이터에 대한 입력쿼리는 쿼리 질의어를 통해 정의된다. 이는 global relation에 대한 쿼리로 번역되고, global conceptual schema를 참조한다.

2. Localization
	- 분산 DB에서, fragmentation은 relation이 분리된 site에 저장되거나, 복사되는데, 
	- 분산 복사된 정보를 이용해 쿼리를 분리한다.
