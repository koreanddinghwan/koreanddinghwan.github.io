...
- `Relational Database Schema(관계형 DB 스키마)`
  ...
  - `S = `
    -  `{R1, R2, ... Rm}`(각 relation schema의 집합)
    - `+ Integrity Constraint(IC)`(integrity constraints의 집합)
...
- `relational db = relational database schema + relational database instance`
...
### `referential integrity constraint(DDL)`
...
### semantic integrity constraint(NOT DDL)
...
- Mini-world의 제약을 반영한다.
  ...
  - DDL의 영역이 아니기에, 다른 방식으로 적용되게된다.
    ...
    - 보통, `CREATE ASSERTION, CREATE TRIGGER`로 사용된다.
...
- UPDATE(MODIFY)는 tuple을 `DELETE한 후, INSERT`하는 것과 같다.
...
- transaction 1개가 끝나면 DB는 valid한 상태로 있어야한다.
