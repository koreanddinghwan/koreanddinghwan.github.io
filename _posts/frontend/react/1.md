...
2. shouldComponentUpdate : 이전 state,props와 현재 state,props를 비교가능
3. render : 렌더링할때 사용하는 메서드
4. getSnapshotBeforeUpdate : 업데이트 이전에 호출
...
render에서 만들어진 결과물이 브라우저에 실제로 반영되기 전에 호출된다.  
