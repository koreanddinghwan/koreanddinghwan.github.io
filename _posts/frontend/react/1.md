...
리덕스에서 사용하는 action 객체에는 type이 필수이다.  
여기서 배우는 useReducer의 action 객체는 type이 필수가 아니고, 객체타입뿐만아니라 문자열, 숫자도 상관없다.
...
useReducer의 첫번째 인자로 reducer 함수를 넣고, 두번째 인자로 기본값을 넣어준다.  
그러면 state값과 dispatch라는 함수를 받아온다.

state는 useReducer에서 기본값으로 넣은 값의 현재상태를 의미하고, dispatch는 액션을 발생시키는 함수이다.
...
useReducer에 reducer함수와 기본상태를 파라미터로 넣어준다. 그러면 state와 dispatch함수를 받아올 수 있다.
...
리듀서함수는 현재 state와 action을 인자로 가진다.  
리턴값으로 새로운 state를 만들어주는데, 기존 state를 spread문법으로 풀어준다.(불변성 유지)

input의 값이 바뀔때마다 onChange이벤트가 발생하는데, 여기서 e.target.value가 변경된 값을 의미한다.

따라서 액션을 발생시키는 함수 dispatch의 인자로 e.target을 넣어주면 reducer함수 내에서 인자로받은 `e.target이 action`이 되면서  
`[action.name]: action.value` -> `e.target.name: e.target.value`가 된다.

## 8.4 usememo

컴포넌트 내부의 연산을 최적화한다.

리스트에 숫자를 추가하면 추가된 숫자의 평균을 보여주는 함수형 컴포넌트를 구현
...
이 컴포넌트의 문제점은 onChange이벤트 리스너가 호출되면서 state에 변화를 가져오고, 컴포넌트가 리렌더링되면서 `getAverage를 다시 호출한다.`라는 것이다.

input에 값을 입력하는동안은 getAverage가 다시 호출되지 않도록 해야한다. 이를 위해 useMemo가 있다.
...
useMemo의 첫번째 인자로는 콜백함수를, 두번째 인자로는 감시할 값을 선택한다.

`감시하고있는 값이 바뀔때만 함수가 호출되고, 감시하고있는 값이 바뀌지 않는다면 함수를 호출하지 않고 이전 값을 리턴한다.`

## 8.5 useCallback

useMemo와 비슷하다.  
useMemo는 특정 값을 감시하고, 그 값이 바뀔때만 함수를 호출하고 값을 업데이트하는데,

`이 훅을 이용하면 만들어놓은 함수를 재사용할 수 있다.`

위의 컴포넌트에서 onChange나 onInsert함수를 적용할 수 있다.  
이 함수들은 컴포넌트가 렌더링될때마다 새로 만들어진 함수를 이용하기 때문에 렌더링을 자주하거나 렌더링할 컴포넌트가 많다면 최적화에 문제가 생긴다.
...
useCallback의 첫번째 인자로 콜백함수를, 두번째 인자로 감시할 state를 넣어준다.

비어있는 배열을 인자로 넣어주면 컴포넌트가 렌더링될때 만들었던 함수를 `재사용`하지만, 배열에 state값을 넣어주면 그 state가 바뀔때마다 새로 만들어진 함수를 사용한다.

두번째 인자로 주어지는 배열에는 해당 함수 내부에서 의존하는 state를 넣어준다.
...
버튼이 클릭될때마다 input으로 focus가 자동으로 넘어가게끔 `inputE1.current.focus()`를 달아준다.

<br><br>

### 8.6.1 로컬변수 사용하기

useRef는 DOM에 직접접근하는 용도 외에 컴포넌트 안에서 조회하고, `수정할 수 있는 변수를 관리하는 훅`으로 쓰인다.

<br>

클래스형 컴포넌트에서는 가변값을 유지하기위해 인스턴스필드에서 선언했는데, `함수형 컴포넌트에서는 useRef()`를 통해 가변값을 유지한다.

useRef는 순수 자바스크립트 객체를 생성하기때문에 매번 렌더링 할 때, `동일한 ref객체`를 만들어준다.

컴포넌트에서 state로 선언된 변수는 세터함수를 호출하고, 렌더링된 이후로 상태를 조회할 수 있지만 `useRef`로 관리되는 변수는 설정 이후 `바로 조회`할 수 있다.
...
단, .current로 값을 바꾼다고하더라도 리렌더링을 유발하지는 않는다.
...
input은 value에 props로 전달된 state값을 업데이트하기위해 name을 주고, onChange에서 setValue([e.target.name] : e.target.value) 이런 방식으로 value값을 업데이트했다.
