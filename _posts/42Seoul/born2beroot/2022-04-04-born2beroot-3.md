...
ssh server를 설치하면 기본적으로 22번 port에 연결되도록 약속되어있다.
web server를 설치하면 80번 포트에 연결되도록 약속되어있다.
따라서 client 컴퓨터를 통해서 웹브라우저에 접속하면, 웹 브라우저는 웹서버의 80번 포트를 알아서 따라간다.
이후 그 결과를 웹 브라우저에 돌려준다.
ssh client는 22번 포트를 사용하도록 약속되어있기에 ssh server에 접속할때 22번 port를 사용하게된다.
