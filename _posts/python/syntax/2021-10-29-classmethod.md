...
클래스의 인스턴스 속성은  `__init__(self)`메서드 안에서 self.속성에 값을 할당한다.
...
()괄호 안에 self가 들어가있는 것을 인스턴스 메서드라고한다.  
...
self.으로 시작하는 속성은 인스턴스 속성,
매개변수로 self가 들어가는 메서드는 인스턴스 메서드가된다.
<br>
<br>
...
1. 정적 메서드는 self를 받지 않기때문에 인스턴스 속성에 접근할 수 없다.(self.으로 접근불가)
...
예를들어, 메서드가 클래스의 인스턴스를 반환해야할때,  
클래스 메서드를 사용하면 cls(value)로 인스턴스를 리턴할 수 있다는 의미이다.  
